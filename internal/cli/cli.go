...
func (c *CLI) handleScan(args []string) error {
	...
	// Perform the scan
	result, err := c.engine.ScanFile(*filePath, opts)
	if err != nil {
	...
	if *outputFormat == "json" {
		...
		c.outputText(result)
		if *reportOutput != "" {
			return c.saveJSONReport(result, *reportOutput)
		}
	}
...
func (c *CLI) outputJSON(result *model.ScanResult, reportPath string) error {
	...
	if reportPath != "" {
		return c.saveJSONReport(result, reportPath)
	}

	return nil
}

// saveJSONReport writes scan result to a JSON file.
func (c *CLI) saveJSONReport(result *model.ScanResult, filePath string) error {
	data, err := json.MarshalIndent(result, "", "  ")
	if err != nil {
		return fmt.Errorf("failed to marshal JSON: %w", err)
	}
